...
export default function AdminWorkspace({ sectionOverride = null }) {
  ...
  const staffRoleScope = activeSection === "drivers" ? "DRIVER" : activeSection === "helpers" ? "HELPER" : activeSection === "settings" ? "ADMIN" : "ALL";
  const visibleStaffUsers = useMemo(() => (
    staffRoleScope === "ALL" ? staffUsers : staffUsers.filter((item) => item.role === staffRoleScope)
  ), [staffRoleScope, staffUsers]);
  const userSectionLabel = activeSection === "drivers" ? "Driver" : activeSection === "helpers" ? "Helper" : activeSection === "settings" ? "Admin" : "Staff";
  ...
  return (
      <div className={`min-h-screen font-sans transition-colors duration-200 ${t.page}`}>
        ...
        {/* ROUTES */}
        {(activeSection === "routes" || activeSection === "stops") && (
        ...
        {/* ACTIVITY */}
        {(activeSection === "analytics" || activeSection === "reports") && (
          <div className="grid gap-5 xl:grid-cols-2">
            <div>
        ...
        {(activeSection === "drivers" || activeSection === "helpers" || activeSection === "settings") && (
          <div className="space-y-4">
              <GlassCard t={t}>
                <div className="mb-4 flex items-start justify-between gap-3">
                  <div>
                    <SLabel t={t}>{editingUserId ? `Edit ${userSectionLabel} Account` : `Add ${userSectionLabel} Account`}</SLabel>
                  </div>
                  {editingUserId ? (
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                    {["DRIVER", "HELPER", "ADMIN"].filter((role) => staffRoleScope === "ALL" || role === staffRoleScope).map(role => (
              ...
              <GlassCard t={t}>
                <div className="flex items-center justify-between mb-3">
                  <SLabel t={t}>{userSectionLabel} Accounts ({visibleStaffUsers.length})</SLabel>
                  <div className="flex gap-1">
                    {["ALL", "DRIVER", "HELPER", "ADMIN"].filter((role) => staffRoleScope === "ALL" || role === "ALL" || role === staffRoleScope).map(r => (
                ...
                <div className="max-h-[48rem] overflow-y-auto space-y-3 pr-1">
                  {visibleStaffUsers.length === 0 && <p className={`text-sm ${t.textSub}`}>No {userSectionLabel.toLowerCase()} users loaded.</p>}
                  {visibleStaffUsers.map(u => (
